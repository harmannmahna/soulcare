...
async def upsert_period(body: CycleBody, user: dict = Depends(require_user)):
    existing = await store.collection("cycles").find_one({"user_id": user["id"]}) or {}
    days = body.days if body.days is not None else existing.get("days") or []
    days = sorted(set(days))
    last_start = body.last_start or (days[0] if days else existing.get("last_start"))
    last_end = body.last_end or (days[-1] if days else existing.get("last_end"))
    doc = {
        "id": f"cyc_{user['id']}",
        "user_id": user["id"],
...
TOPICS = ("mental-health", "women", "habits", "physical", "study", "kindness")


@router.get("/community")
async def community_feed(topic: str | None = None):
    query = {}
    if topic and topic != "all":
        query["topic"] = topic
    rows = await store.collection("community").find(query, sort=[("created_at", -1)], limit=40)
    return [_clean(r) for r in rows]


@router.get("/community/topics")
async def community_topics():
    return [{"id": t, "label": t.replace("-", " ").title()} for t in TOPICS]
...
async def community_post(body: CommunityBody, user: dict = Depends(require_user)):
    ...
    topic = body.topic if body.topic in TOPICS else "mental-health"
    doc = {
